#include "Building/BlockoutRoad.h"
...
ABlockoutRoad::ABlockoutRoad()
{
	...
	BlockoutBuildingShapeUtils::InitializeSpline(
		...
		false,
		EBlockoutSplinePointType::Curve);
}

void ABlockoutRoad::CPPGenerateBlockoutMesh()
{
	...
	UDynamicMesh* RoadMesh = AllocateComputeMesh();
	BlockoutBuildingShapeUtils::AppendSplineRibbonFromPoints(RoadMesh, SplinePoints, SourceSpline->IsClosedLoop(), RoadWidth, Alignment);

	FGeometryScriptMeshExtrudeOptions ExtrudeOptions;
	ExtrudeOptions.ExtrudeDistance = FMath::Max(RoadThickness, 0.01f);
	ExtrudeOptions.ExtrudeDirection = FVector(0.0f, 0.0f, 1.0f);
	ExtrudeOptions.UVScale = 1.0f;
	ExtrudeOptions.bSolidsToShells = true;
	UGeometryScriptLibrary_MeshModelingFunctions::ApplyMeshExtrude_Compatibility_5p0(RoadMesh, ExtrudeOptions);
